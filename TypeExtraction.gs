...
function processUploadedPDF(formObject) {
  ...
  let tempDocId = null; // Declared outside the try block so the finally block can access it
  ...
  try {
    ...
    // Guaranteed Cleanup: Runs whether extraction succeeds or throws an error
    if (tempDocId) {
      try {
